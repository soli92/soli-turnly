...
describe('validateMinRest (RB-02)', () => {
  describe('riposo sufficiente', () => {
    it('12h di riposo dopo turno precedente — nessuna violation', () => {
      ...
      const existing = [
        makeShift('s1', USER_A, '2025-06-10T00:00:00Z', '2025-06-10T08:00:00Z'),
      ];
  ...
  describe('riposo insufficiente (modalità default WARNING)', () => {
    ...
    it('riposo insufficiente prima del turno successivo — WARNING', () => {
      ...
      const existing = [
        makeShift('s2', USER_A, '2025-06-11T16:00:00Z', '2025-06-12T00:00:00Z'),
      ];
  ...
  describe('modalità STRICT (MIN_REST_STRICT=true)', () => {
    beforeEach(() => {
      process.env['MIN_REST_STRICT'] = 'true';
    });

    afterEach(() => {
      delete process.env['MIN_REST_STRICT'];
    });
