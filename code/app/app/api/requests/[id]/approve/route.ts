...
export async function POST(req: Request, { params }: RouteParams): Promise<Response> {
  ...
  // dopo che la Response è già stata inviata al client (Next.js 15 API).
  after(async () => {
    try {
      ...
      if (requester?.email) {
        await inngest.send({
          ...
          data: {
            to: requester.email,
            subject: 'La tua richiesta è stata approvata — Turnly',
      ...
      console.error('[TSK-029] dispatch request-approved email failed', err);
    }
  });
