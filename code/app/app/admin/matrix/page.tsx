...
function parseISOWeekRange(weekStr: string): { start: Date; end: Date } {
  ...
  // match[1] and match[2] are guaranteed by the regex above (noUncheckedIndexedAccess guard)
...
export default async function MatrixPage({ searchParams }: MatrixPageProps) {
  ...
  const [employeesResult, shiftsResult, shiftTypesResult, absencesResult] = await Promise.all([
    ...
    db
      ...
      .from(shiftsTable)
      .leftJoin(shiftTypesTable, eq(shiftsTable.shiftTypeId, shiftTypesTable.id))
      .where(and(gte(shiftsTable.date, startDateStr), lte(shiftsTable.date, endDateStr))),
    ...
    db
      ...
      .from(absencesTable)
      .innerJoin(absenceTypesTable, eq(absencesTable.absenceTypeId, absenceTypesTable.id))
      .where(
        and(
  ...
  const qualificationsForFilter = Array.from(
    new Map(
      employees
        ...
        .map((e) => [e.qualificationId, { id: e.qualificationId!, name: e.qualificationName! }])
    ).values()
  );
